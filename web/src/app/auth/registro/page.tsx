...
import {
  Eye, EyeOff, User, Mail, Lock, Phone, MapPin,
  ChevronLeft, Check,
} from "lucide-react";
  ...
  return (
    <div className="w-full max-w-[420px] text-center">
      <div
        ...
        style={{
          background: "rgba(243,158,16,0.10)",
          border: "1.5px solid rgba(243,158,16,0.22)",
        }}
      >
        <Mail size={28} style={{ color: "#f39e10" }} strokeWidth={1.6} />
      </div>

      <h2 className="text-gray-900 text-[22px] font-black tracking-[-0.4px] mb-2">
        Revisa tu correo
      </h2>
      <p className="text-gray-500 text-[14px] leading-relaxed mb-1">
        Enviamos un enlace de verificación a
      </p>
      <p className="text-gray-900 font-semibold text-[14px] mb-6 break-all">{email}</p>

      <div
        className="rounded-xl p-4 mb-6 text-left"
        style={{
          background: "rgba(243,158,16,0.06)",
          border: "1px solid rgba(243,158,16,0.15)",
        }}
      >
        <p className="text-[13px] text-gray-600 leading-relaxed">
          <span className="font-semibold text-gray-800">Próximo paso:</span> Una vez
          verificado tu correo, continúa configurando el perfil de tu negocio (Paso 2 de 5).
        </p>
      </div>
      ...
      <button
        ...
        disabled={resent}
        className="w-full border border-gray-200 rounded-xl py-3 text-[14px] font-semibold text-gray-600 hover:bg-gray-50 transition-colors mb-3 cursor-pointer bg-white"
        style={{ opacity: resent ? 0.7 : 1 }}
      >
...
function RegistroForm() {
  ...
  const handleGoogleOAuth = async () => {
    ...
    const { error: oauthError } = await getSupabase().auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/callback?role=provider&next=/proveedor`,
      },
    });
    if (oauthError) {
  ...
  if (isProvider && verifying) {
    return <VerificationScreen email={form.email} onResend={handleResend} />;
  }
    ...
    <div className={`w-full ${isProvider ? "max-w-[480px]" : "max-w-[440px]"}`}>
      ...
      <div className="mb-7">
        ...
        <p className="text-gray-500 text-[14px]">
          {isProvider
            ? "Datos de acceso. Tu perfil de negocio lo configuramos en el siguiente paso."
